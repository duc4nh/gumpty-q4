...
public class BusInfo 
{
  ...
  {
    return database.busDatabase.select_ids("bus_id", "bus", "workload");
  }
  ...
  public static int getWorkload(int busId) {
	  if (busId == 0)
		  throw new InvalidQueryException("Invalid bus id: 0");
	  return database.busDatabase.get_int("bus", busId, "workload");
  }
  
  public static void setWorkload(int busId, int workload) {
	  if (busId == 0)
		  throw new InvalidQueryException("Invalid bus 0");
	  database.busDatabase.set_value("bus", busId, "workload", workload);
  }
